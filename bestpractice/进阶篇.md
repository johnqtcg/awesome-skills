...
6. [高质量 Skill 的设计模式](#6-高质量-skill-的设计模式)
   - [6.1 强制门禁架构（Mandatory Gates）](#61-强制门禁架构mandatory-gates)
   - [6.2 反例教学（Anti-Examples）](#62-反例教学anti-examples)
   - [6.3 三级质量记分卡（Three-Tier Scorecard）](#63-三级质量记分卡three-tier-scorecard)
   - [6.4 黄金用例与合约测试（Golden Fixtures + Contract Tests）](#64-黄金用例与合约测试golden-fixtures--contract-tests)
   - [6.5 结构化输出契约（Structured Output Contract）](#65-结构化输出契约structured-output-contract)
   - [6.6 版本与平台感知（Version/Platform Awareness）](#66-版本与平台感知versionplatform-awareness)
   - [6.7 诚实降级（Honest Degradation）](#67-诚实降级honest-degradation)
   - [6.8 自由度分级（Degrees of Freedom）](#68-自由度分级degrees-of-freedom)
   - [6.9 五种执行编排模式（来自 Anthropic 官方指南）](#69-五种执行编排模式来自-anthropic-官方指南)
7. [常见陷阱与反模式](#7-常见陷阱与反模式)
   - [7.1 Description 决定 Skill 的生死](#71-description-决定-skill-的生死)
   - [7.2 SKILL.md 超过 500 行](#72-skillmd-超过-500-行)
   - [7.3 Reference 文件没有加载条件](#73-reference-文件没有加载条件)
   - [7.4 没有反例，只有正例](#74-没有反例只有正例)
   - [7.5 忽视 `allowed-tools` 安全约束](#75-忽视-allowed-tools-安全约束)
   - [7.6 动态上下文注入的误用与正用](#76-动态上下文注入的误用与正用)
   - [7.7 创建多余文件](#77-创建多余文件)
   - [7.8 命名与安全硬限制](#78-命名与安全硬限制)
   - [7.9 性能与加载限制](#79-性能与加载限制)
   - [7.10 常见误解](#710-常见误解)
8. [实战案例：从简单到复杂](#8-实战案例从简单到复杂)
   - [8.1 简单案例：git-commit](#81-简单案例git-commit)
   - [8.2 复杂案例：go-code-reviewer](#82-复杂案例go-code-reviewer)
9. [设计哲学：从可传授到可执行](#9-设计哲学从可传授到可执行)
   - [9.1 知识的三种形态](#91-知识的三种形态)
   - [9.2 案例：git-commit skill 与Git规范的对齐](#92-案例git-commit-skill-与git规范的对齐)
   - [9.3 同一哲学在不同 Skill 中的体现](#93-同一哲学在不同-skill-中的体现)
   - [9.4 三个关键能力](#94-三个关键能力)
   - [9.5 三条设计原则](#95-三条设计原则)
...
门禁是 skill 最核心的质量保障机制——**不满足前置条件，不允许继续执行**。门禁数量从 4 个（postgresql-best-practise）到 8 个（create-pr）不等。
...
`git-commit` skill（§8.1）是这一哲学最直观的实证。团队在 [productivetools/git](../../productivetools/git/) 第 2 章《日常操作命令详解》和第 6 章《工作流与提交规范》中沉淀了完整的 Git 提交规范，skill 将其逐条转化为 7 步工作流中的强制门禁：

| Git 文档规范 | 出处 | Skill 对齐状态 |
|-------------|------|---------------|
| 原子性提交：一次 commit 只做一件事 | 第 2 章 §2.3 | ✅ Hard Rule + `git add -p` 默认策略 |
| 描述"为什么"而非"做了什么" | 第 2 章 §2.3 | ✅ body 必须回答 "why it changed" |
| 遵循团队 commit 规范 | 第 2 章 §2.3 | ✅ Conventional Commits 全套规则 |
| Conventional Commits 格式 | 第 6 章 §2.1 | ✅ type(scope): subject 完全一致 |
| type 类型覆盖 | 第 6 章 §2.2 | ✅ 超集（额外 +build/revert） |
| subject ≤ 50 字符 | 第 6 章 §2.4 | ✅ 与 GitHub 截断阈值对齐 |
| body 解释"为什么改" | 第 6 章 §2.5 | ✅ what vs why 职责分离 |
| footer 规范 | 第 6 章 §2.6 | ✅ BREAKING CHANGE + Closes # + Refs: |
| `--amend` 安全警告 | 第 2 章 §2.2 | ✅ 含已 push 场景的 force push 风险提示 |

9 条规范全部覆盖，从原子性提交到 footer 格式，从 subject 长度限制到 `--amend` 安全警告，无一遗漏。关键转变不在于 skill 重复了文档的内容，而在于它改变了知识的**执行方式**：从"请参考第 6 章 §2"变成了"不符合规范就不允许提交"。
...
| unit-test | "测试应该发现 bug，不是追覆盖率" | Defect-First Workflow 方法论文档 | 写测试前必须先产出失败假设清单，每个目标必含 Killer Case |
