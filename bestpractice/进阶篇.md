...
- [6. 高质量 Skill 的设计模式](#6-skill)
    ...
    - [6.10 边界验证证据](#610-boundary-evidence)
- [7. 常见陷阱与反模式](#7)
...
- [8. 实战案例：从简单到复杂](#8)
    ...
    - [8.3 案例深挖：git-commit 的显性化路径](#83-git-commit)
...
通过生产级 skill 评审，提炼出 **9 个质量保障模式**（6.1–6.8 与 6.10）。此外，Anthropic 官方指南归纳了 **5 种执行编排模式**（6.9），两者正交互补——前者管"做得好不好"，后者管"怎么编排执行"。
...
| 6.10 | 边界验证证据 | 结构化记录只是声明，必须由 Git、文件系统、API 或真实进程独立验证 | `deep-research` |
...
注意：在 skill 中，"结论先行"不止是摘要先行，连**审查深度（Lite/Standard/Strict）**都在第一节声明——读者在看任何 finding 之前就已经知道本次审查的覆盖范围。
...
这条原则管的是输出的**真实性**——其他四条原则都假设输出是真的，而执行完整性原则保证这个假设成立。10/10 个生产级 skill 都将它作为硬门禁写入 Output Contract。
...
补充：当 finding 数量超出软上限（如 Standard 模式最多 10 条）时，超出部分**不能静默丢弃**，统一移入 Residual Risk 节，并注明"N additional findings deferred to Residual Risk"。
...
实现手段：
- **JSON summary block**（Standard/Strict 模式）：提供机器可解析的摘要，CI 可直接 `jq` 提取
- **PASS/FAIL verdict**：布尔值，不是"较好""有问题"之类的模糊描述
- **字段名稳定性**：字段名一旦发布不能随意改名——下游脚本依赖它们，改名等于 breaking change
...
<a id="610-boundary-evidence"></a>
### 6.10 边界验证证据

格式完整的 artifact 不等于可信证据。像
`{"commit":"deadbee","status":"passed"}` 这样的 JSON，只能证明有人写入了
字段。生产级 skill 在使用记录之前，必须跨越对应的权威边界：

| 声明 | 权威校验 |
|---|---|
| 代码属于某个 revision | 解析 commit、回读声明的 blob，并核对 path/line/excerpt |
| 当前文件观察 | 把路径限制在声明 root 内；脏文件或未跟踪内容必须标记为未固定 |
| 测试支持运行时主张 | 通过宿主权限层只执行一次；要求一个 receipt 覆盖 claim 与同一快照下完整的 pinned 代码集、tested paths 和相关性审核 |
| Web 页面是实时一手来源 | 忽略导入的 tier/live 标签；由验证器抓取、匹配摘录，并根据有效最终 URL 推导权威等级 |
| 会话预算仍有余额 | 在加锁的持久化状态中预留用量，而不是只检查单次命令 |

采集与验证应相互分离。被广泛允许的 validator 不能成为通用命令代理；
它应导入宿主生成的 receipt 并执行静态边界校验。报告必须从规范化后的
已验证记录渲染，而不是复述原始 artifact 里的不可信 summary。这样才能
防止“看起来像证据的 schema”或与主张无关的 exit code 0 变成伪造执行状态。
质量过滤必须发生在完整引用证据集校验之后；否则一个 pinned 项可能掩盖
另一个 unpinned 项，多个局部 receipt 也可能被拼接成没有任何一次执行真正
支持的置信度。可以提供只读 snapshot helper 暴露 commit/tree/dirty 身份，
但它不能执行记录中的命令，也不能声称独立证明了历史执行。

同一边界原则也适用于网络。被广泛允许的研究 helper 不能变成 SSRF 或本地
文件代理：只允许公网 HTTP(S)，任一 DNS 答案非公网即拒绝，连接已经验证的
IP 并保留 TLS 主机名校验，每个重定向都重新验证。保存的正文及其 hash
仍只是可审计输入，不能作为后续验证器确实执行过抓取的可复用证明。
...
大多数 skill 只给正面规范：告诉 AI "好的提交长什么样"。`git-commit` 的 Prompt 设计走得更远——同时列出了 **5 个工程痛点**作为反面约束：
...
| 操作 | 约束类型 | 具体规则 |
|------|---------|---------|
| subject 长度 | 精确硬约束 | ≤ 50 字符（GitHub 截断阈值） |
| body 每行长度 | 精确硬约束 | ≤ 72 字符（终端 `git log` 可读性） |
| scope 发现 | 算法规则 | 扫描近 30 条历史，出现 ≥ 3 次才使用 |
| body 内容 | 原则性描述 | "解释为什么改，而非重复描述做了什么" |
| footer | 原则性描述 | "BREAKING CHANGE、Closes #、Refs: 均可选" |
