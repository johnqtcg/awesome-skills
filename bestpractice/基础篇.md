...
1. [Skill 出现的背景](#1-skill-出现的背景)
   - [1.1 AI 编程助手的上下文困境](#11-ai-编程助手的上下文困境)
   - [1.2 从 prompt 到可复用的知识单元](#12-从-prompt-到可复用的知识单元)
2. [Skill 基本介绍](#2-skill-基本介绍)
   - [2.1 一个 Skill 长什么样](#21-一个-skill-长什么样)
   - [2.2 两种分类维度](#22-两种分类维度)
   - [2.3 调用方式](#23-调用方式)
   - [2.4 Frontmatter 字段参考](#24-frontmatter-字段参考)
3. [部署位置与适用场景](#3-部署位置与适用场景)
   - [3.1 什么时候不需要 Skill](#31-什么时候不需要-skill)
   - [3.2 创建你的第一个 Skill：先迭代，再提炼](#32-创建你的第一个-skill先迭代再提炼)
   - [3.3 分发渠道：从本地到 API](#33-分发渠道从本地到-api)
4. [进阶结构：封装脚本与辅助文档](#4-进阶结构封装脚本与辅助文档)
   - [4.1 各目录的职责](#41-各目录的职责)
   - [4.2 脚本封装的价值](#42-脚本封装的价值)
5. [渐进式披露：解决 AI 上下文瓶颈的优雅之道](#5-渐进式披露解决-ai-上下文瓶颈的优雅之道)
   - [5.1 核心原理](#51-核心原理)
   - [5.2 选择性加载表](#52-选择性加载表)
   - [5.3 长文件加 Table of Contents](#53-长文件加-table-of-contents)
