...
> **Platform constraint**: Claude Code does not allow subagents to spawn other subagents ("Subagents cannot spawn other subagents"). The `go-review-lead` orchestrator therefore runs as a **Skill in the main conversation**, not as an agent definition in `.claude/agents/`. The 8 Worker Agents in `agents/` are dispatched by the main conversation, not by a Lead Agent.

## What's in This Directory

```
agents/                            # 8 vertical Worker Agents only
...
└── go-observability-reviewer.md   # Worker: structured logging, trace propagation, metric cardinality
```
...
| `go-observability-reviewer.md` | `go-observability-review` skill |

The `go-review-lead` skill is loaded by the main conversation — it is the orchestrator, not a worker.

The source files for all 9 skills (1 orchestrator + 8 verticals) are in the `skills/` directory of this repository (e.g. `skills/go-concurrency-review/SKILL.md`).
...
Copy **only the 8 Worker Agent** definition files to `~/.claude/agents/`. Do **not** copy `go-review-lead.md` — it does not belong in `agents/`.
...
# Expected output (8 workers, no go-review-lead):
# go-concurrency-reviewer.md
# go-error-reviewer.md
# go-logic-reviewer.md
# go-observability-reviewer.md
