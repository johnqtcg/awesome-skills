...
> **评估后更新 — 2026-07-19：** 当前实现已在文档和报告生成器中统一为唯一九节合同。Web/Hybrid 强制要求 `content.json`、精确摘录匹配与报告自动验证，但保存的正文被明确视为调用方输入：加载时清除 live 状态，调用方提供的 tier/type/domain 不能授予权威。Web High 现在必须由当前 `validate/report --live-web` 实时抓取，并根据有效最终 URL 重新推导 T1。所有内置 Web 访问都经过公网限定 HTTP(S) 传输：拒绝本地 scheme、URL 凭据、localhost、任一非公网或混合 DNS 答案及不安全重定向；连接已验证 IP，同时保留 TLS 主机名校验。仓库结论继续独立回读真实 Git 对象/blob 或受根目录约束的未固定工作区内容；测试仍由宿主权限层执行，一个版本化 receipt 把完整引用代码集绑定到同一干净快照。模式预算在跨进程锁保护的会话账本中累计，外部工具有预算预留入口，引用来源上限可执行。离线回归现包含 301 项测试，覆盖调用方伪造 T1/live、本地文件/私网地址/混合 DNS/重定向、DNS 固定建连、request → decision、伪造仓库证据、真实多进程预算竞争和来源上限。Receipt 仍是宿主 attestation 而非密码学执行证明；序列化 Web hash 仍是审计元数据而非可复用执行证明；账本是运行约束而非防篡改日志；Windows 锁仍需 Windows CI。这些改进不属于 2026-03-12 原始评分，后续应通过全新 benchmark 重新评估。
