...
> **适用范围说明（2026-07-31 补充）。** 下列分数测的是 2026-03-19 当时的 `update-doc`。
> 那个版本只用 `git diff --name-only` 判定变更范围，而该命令只报告已跟踪文件的未暂存
> 改动，暂存过的改动和新建文件根本不在闸门视野内。因此 §7.2 中“项目类型路由与 diff
> 范围 10/10”这一项反映的是路由行为，diff 范围部分并未被真正验证过。该闸门此后已替换为
> `scripts/discover_doc_scope.sh`，合并四个变更来源。这些数字**没有**针对当前版本重新测量。
