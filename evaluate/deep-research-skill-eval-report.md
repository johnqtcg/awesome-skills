...
> **Post-evaluation update — 2026-07-18:** The current implementation now enforces one nine-section contract in both documentation and the report generator; requires `content.json` for web/hybrid validation and reporting; verifies exact supporting excerpts; auto-validates reports; applies one High-confidence rule; and uses conservative T1–T5 source metadata. Repository claims are checked against real Git objects/blobs or bounded unpinned working-tree content. Test commands execute once through the host permission layer; the helper only imports a versioned receipt. Runtime High validates the complete cited-code set before quality filtering: every item must verify and be pinned to one commit/tree, and one receipt must cover the finding, all code IDs, and all tested paths. A read-only `snapshot-codebase` command emits HEAD/tree/dirty identity without executing tests and rejects output paths that would dirty the measured repository. The former command proxy and replay flags are gone. Mode budgets accumulate under cross-process locks, external tool calls have a reservation path, and report-source ceilings apply only to cited evidence. The offline regression now contains 286 tests, including request → decision, forged evidence, generic-success-command rejection, unresolved/unpinned-mixing/split-receipt/snapshot negatives, a real multiprocess budget race, and source-ceiling checks. Receipts remain host attestations rather than cryptographic execution proof; the ledger is an operational constraint rather than a tamper-proof log; and Windows locking still needs Windows CI. These improvements were not part of the 2026-03-12 score and should be evaluated in a future fresh benchmark.
