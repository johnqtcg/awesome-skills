...
快照说明：本报告评估的是 **2026-03-17** 时点的 `tech-doc-writer` skill。此后门禁编号已经变化：被评估的快照是五道门禁，质量评分卡是 Gate 4；当前 skill 是四道门禁（0 执行真实性、1 仓库上下文扫描、2 文档类型与受众分类、3 质量评分卡）。下文提到的 "Gate 4: Quality Scorecard" 对应现在的 **Gate 3**，旧文中的 "Gate 1 和 Gate 3" 对应现在的 Gate 1 和 Gate 2。结构数据（SKILL.md 行数、token 估算）和 **9.01/10** 的加权总分都只对应被评估的那个快照，不是当前版本。

> **首次真实模型 with/without 对照评测（2026-07-30）—— 9.01 分未被复现。** 用真实模型跑了一次对照评测（`claude -p --model sonnet`，3 个场景 × 2 组，每格 1 个样本）。**两组都没有产出评分器认可的文档：带 skill 0/3，不带 skill 0/3**，原始失败项计数为带 skill 8 项、不带 skill 7 项。其中两项逐项差异明确对 skill 有利：基础模型完全不会自发给出适用项分母（3/3 失败 vs 1/3），以及最小差异改写的超额量减半（改动 57 行 vs 112 行，预算 40 行）；而"引用文件未加载"这一项是评分器本身的不对称，不是能力退化。完整细节，包括这次运行暴露出的 5 个 harness 缺陷，见 `skills/tech-doc-writer/scripts/tests/COVERAGE.md`。在完成一次多样本的新基准测试之前，下文的 9.01/10 应视为未被复现。

> **评估后更新 —— 2026-07-30：** 上面打分涉及的两项能力此后被重写，所以 9.01 这个分数在几个维度上偏低、在一个维度上偏高。打分时的"防陈旧"只检查元数据字段在不在 —— `last_updated: 2000-01-01` 零告警通过 —— 现在会拿文档年龄和声明的 `review_cadence` 做比较。机械检查层新增了参数表必需列检查、CommonMark `~~~` 与嵌套围栏处理、元数据 title 与 H1 一致性检查，以及"两个空格"这一类盘古空格违规的识别。Gate 0 增加了三级验证阶梯（已执行 / 有出处 / 未验证），原因是此前那条 Critical 要求命令必须实际跑过，而工具授权跑不了 `kubectl`、`docker` 这类命令，导致大部分 runbook 根本不可能达到最高等级。仓库自身规范现在通过 `.techdocrc.json` 强制执行，不再是 Gate 1 承认它优先、Phase 4 又把它判成错误。评分卡里原本一刀切的条目（"零术语混用"、所有类型都要 80% 结构化信息）现在按文档类型触发。2026-07-30 首次对真实模型跑了 with/without-skill 对照评测，见 `scripts/tests/ab_eval.py` 和 `scripts/tests/COVERAGE.md`。这些改动都没有体现在下面的分数里，应在新一轮 benchmark 中重新测量。
