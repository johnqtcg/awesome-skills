...
> **过时提示（2026-08-27）：** 本评估针对的是 `SKILL.md` 的一个前代版本——296 行、只有 3 个场景（见 §5.1 的体量表）。2026-08-27 该 skill 做了一次较大幅度的结构调整：拆分出仅诊断 / 诊断并修复 / P0 三种模式、增加了 N/A 计分规则和安全授权门禁，并修正了若干事实性错误，详见 `rationale/systematic-debugging/design.zh-CN.md` 第 11 节。下面的 8.76/10 分数不能代表当前的 `SKILL.md`。建议后续用同样的 with/without-skill 子代理方法论（见 §2）针对当前文件重新跑一轮真实评估，而不是直接假定分数依然适用。
