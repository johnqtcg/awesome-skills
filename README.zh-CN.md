...
由 `stock-analysis-lead` 编排器对单只美股做七层基本面"X 光"：并行派发六个垂直股票研究 skill 作为子智能体，再综合给出买入/持有/卖出结论。这些 skill 作为一个系统协同工作，复用了 `go-review-lead` 的多智能体模式。范围仅限单只美股的基本面分析——不涉及交易信号、技术分析、期权、加密货币、ETF 或非美股标的。

| Skill 名称 | 角色 | 功能用途 | 主要亮点 / 优势 |
| --- | --- | --- | --- |
| `stock-analysis-lead` | 编排器 | 判定行业原型、拉取 SEC 文件、派发六个垂直子智能体并综合结论 | 用行业感知阈值给出买入/持有/卖出判断和 Bull/Base/Bear 目标价区间；将每次结论写入 JSON-Lines 日志，使后续分析参照此前观点 |
