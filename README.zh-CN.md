...
这一类 skill 把零散的事实和直觉压缩成受众能快速吸收的结构。

| Skill 名称 | 功能用途 | 主要亮点 / 优势 |
| --- | --- | --- |
| `constructive-thinking` | 在输出前把混乱信息压缩成"领导可读"的结构 | 强制 `框架 → 命题 → 节点 → 倾向 + 理由`（Frame → Crux → Nodes → Tilt + Reasoning）的形态，让汇报、技术方案或 one-pager 能在 3–5 分钟读完，而不是平铺一堆事实 |

### 美股投资分析（多智能体系统）

由 `stock-analysis-lead` 编排器对单只美股做七层基本面"X 光"：按分层策略并行派发垂直股票研究 skill 作为子智能体，子智能体通过带版本的 JSON findings 契约回传结果，再综合给出买入/持有/卖出结论。派发是分层的：四个核心评审始终运行，另两个按显式触发条件运行，第二波仅在出现具名的跨 worker 矛盾时才派发。这些 skill 作为一个系统协同工作，复用了 `go-review-lead` 的多智能体模式。范围仅限单只美股的基本面分析——不涉及交易信号、技术分析、期权、加密货币、ETF 或非美股标的。

| Skill 名称 | 角色 | 功能用途 | 主要亮点 / 优势 |
| --- | --- | --- | --- |
| `stock-analysis-lead` | 编排器 | 判定行业原型、拉取 SEC 文件、按分层派发垂直子智能体、逐条校验回传结果是否符合 findings 契约，再综合结论 | 用行业感知阈值给出买入/持有/卖出判断和 Bull/Base/Bear 目标价区间；将每次结论写入 JSON-Lines 日志，使后续分析参照此前观点 |
| `stock-business-review` | 商业模式 | 审查商业模式与收入结构 | 覆盖产品/服务结构、客户集中度、地域敞口、收入增长拆解，以及"事实 vs 管理层叙事"的信息分层纪律 |
| `stock-earnings-quality-review` | L1–L3 | 审查盈利质量、现金流真实性与经营杠杆 | 跟踪经营现金流与净利润的偏离、自由现金流轨迹、资本开支性质、收入质量信号,以及 SaaS 指标（NRR/GRR/CAC 回收期/Magic Number） |
| `stock-balance-sheet-review` | L4 | 审查资产负债表健康度与抗周期能力 | 净债务/EBITDA 杠杆、流动比率、现金可支撑期、商誉集中度、DSO/库存趋势、表外项目、养老金缺口 |
| `stock-management-review` | L5 | 审查管理层质量与资本配置历史 | 5 年资本配置历史、回购时机、并购投资回报、指引 vs 实际的对照、薪酬激励对齐和内部人交易 |
| `stock-industry-review` | L6 | 审查行业地位与竞争护城河 | 波特五力扫描、市场份额趋势、TAM 轨迹、护城河分类（7 种命名类型）和定价权证据 |
| `stock-peer-comparison-review` | 交叉验证 | 将标的独立对标 2–4 个最接近的同业 | 固定 12 项比率面板（增长、盈利能力、资本密集度、杠杆、资本回报、估值），交叉验证护城河与市场份额论断 |
