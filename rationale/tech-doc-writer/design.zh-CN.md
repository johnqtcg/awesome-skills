...
### 4.6 把“怎么定”和“交什么”拆成两条独立的轴

这个 skill 不要求在信息不足时硬写完整文档。它把原先混在一起的一个判断，拆成了两个互不干扰的判断。

**歧义怎么消除 —— 检索 → 询问 → 假设**，顺序固定，前一步失败才走下一步：

| 步骤 | 做什么 |
|---|---|
| **R1 检索** | 在仓库和已有文档里找：现有文档写明的读者、README、`CONTRIBUTING`、ADR、被记录代码的调用方。只做一轮，不穷尽。 |
| **R2 询问** | 问一个合并后的问题，点明缺哪几个具体事实。只问一次，不是每个缺口问一遍。 |
| **R3 假设** | 带着明确标注的假设继续写。 |

**最终交什么 —— 1 级完整 / 2 级部分 / 3 级脚手架。**

关键在于这是两条轴，不是一条。早先的版本在交付等级里塞了一个“2.5 级：主动检索”，等于把“skill 怎么做决定”和“读者拿到什么”混为一谈：检索是一个动作，不是一份交付物，你没法把“2.5 级”交到读者手上。更麻烦的是，Gate 2 写着“受众不清 → STOP and ASK”，而等级表写着“受众不确定 → 按最宽受众假设并继续”，两处都没说谁先谁后：同样的输入，两次运行可以做出完全相反的事。

“先检索再降级”这条思路保留下来了，就是现在的 R1，它依然是这个 skill 和普通写作提示词的分水岭 —— 技术文档的信息缺口，通常不是“信息根本不存在”，而是“信息就在仓库里，只是还没去找”。变化的是：R1 现在是一个状态机里的一步，状态之间怎么跳转有明确规定；走到 R3 必须满足对“无法询问”的收窄定义（用户已授权自行判断、非交互式批处理、或用户已明确拒绝说明 —— 单纯“用户这一轮还没回话”不算）；每次输出都必须写清是哪一步消除了歧义。只声明 `Level 2` 而不写解析路径，属于没有依据的降级，forward-eval 的评分器会直接判失败。
...
| 存在信息缺口但仓库里可能有答案 | 非常适合 | 降级之前必先走 R1 检索 |
