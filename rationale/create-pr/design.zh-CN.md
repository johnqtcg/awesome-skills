...
这条原则要成立，覆盖度必须分维度判定。"发现的命令全部成功"说的是命令，而不是这次变更：在一台没装 `golangci-lint` 的机器上，只汇总退出码的门禁会给出"通过"，而 lint 实际上一次都没跑。因此 D 号门禁把 **test**、**lint**、**build** 三个维度独立结算，每个维度只有三种结论——已执行（附命令）、已声明不适用、未覆盖（附原因）。未覆盖的维度会进入 Uncovered Risk List，并把 PR 保持在 draft。同理，`make ci` 这类不透明的封装命令不计入任何维度：脚本无法观测它实际跑了什么，只能由作者显式声明。在这里靠猜，等于重现这道门禁本来要拦住的问题。

分类依据是命令的 **argv**，而不是命令文本。拿正则去搜整段字符串，会把
`printf '%s' 'go test ./...; golangci-lint run; go build ./...'`——一条只打印三个工具名、
什么也没验证的命令——判成三个维度都已执行，正好造出这道门禁要消灭的那种假"已执行"。
因此只信任 argv[0] 及其子命令 / 任务目标；shell 表达式与封装命令保持未分类。少判会让维度
停留在未覆盖、PR 停留在 draft，这是可以补救的不便；多判会把从未运行的检查报告成已执行，
那才是缺陷。

读 argv 是必要的，但还不够：**命令名正确，运行模式不对，一样什么都没执行。**
`make -n test lint build` 点到了三个维度、打印了三条 recipe，却一条都没跑——给每个目标加一个
marker 文件、观察到没有任何 marker 生成，即可确认。所以当参数把一个已识别的工具切到
dry-run、只列举、跳过、帮助或版本模式时，同样不予采信。这些集合按**工具**维护而不是全局，
因为同一种写法在不同工具里含义相反：`make -n` 是 dry run，`pytest -n 4` 是并行跑测试；
`make -q` 是提问，`mvn -q` 只是安静；`cargo -V` 是版本，`ctest -V` 是 verbose。一份全局
标志清单会在两个方向上同时出错。

这个设计还带出两条代价换来的结论：

- **判断单位是选项字母，而不是参数 token。** `make -sn test` 把 `-s` 和 `-n` 合并进一个 token，
  按整 token 匹配会把一次静默的 dry run 报成三个维度都已执行。逐字母展开只有配上停止规则才成立：
  遇到第一个"把 token 余部当作自己的值"的字母就停止扫描，否则 `make -fMakefile.test` 会被读成
  "含 `-t`"、`mvn -Pdev` 会被读成"含 `-v`"。而且是否允许合并本身就是各工具自己的约定——Go 的
  flag 包不支持合并，所以 `-run` 绝不能被读成 `-r -u -n`。

  这套展开的第一版给自己加了一道"token 必须全是字母"的前置条件，而这正是**一道把检查本身关掉的
  前置条件**，且恰好关在最需要它的输入上：`make -snj2`、`make -nf./Makefile` 在合并标志后面带了
  取值，通不过"全字母"判断，于是整个参数根本没被扫描——前面的 `-n` 就此漏掉，三个维度被记给了
  一条什么都没跑的命令。正确做法是对原始 token 从左到右解析，让停止规则去做判断；**不要让形状
  判断来决定是否要看**。镜像风险在停止集合里：把一个实际不取值的字母列为取值字母
  （`mvn -o` 是 offline），会让扫描提前结束、掩盖它后面的一切——所以那张表由专门的测试断言。
- **显式声明不是豁免。** `quality.test_cmd` 的用途是让作者说明一个不透明封装命令覆盖了哪些维度，
  它并不说明这条命令是否真的运行。因此"归属哪个维度"与"这条命令到底能不能执行检查"是两个独立
  步骤，而*所有*来源——显式声明、`check_cmd`、仓库自动发现——都要经过第二步。拒绝结果会挂在被
  声明的那个维度上报告，作者能准确看到哪一条声明被驳回、以及为什么。无法识别的封装命令仍然成立：
  命令里没有任何东西能反驳这条声明，而这正是允许声明的意义。

而且运行模式根本不必出现在命令里。`MAKEFLAGS=n` 就是 `make -n`——make 把这个变量文档化为开关
列表，其中打头的裸词还是单字母的合并写法。于是同一个判定又多了两个产生者：写在命令里的赋值
（`make MAKEFLAGS=n test`，或作为 shell 前缀）以及执行用的 shell **直接继承**到的值。后者甚至
不需要任何自定义命令：只要导出了 `MAKEFLAGS=n`，自动发现出来的 `make test` / `make lint` /
`make build` 就是三次 dry run，而命令文本干干净净。因此这项检查会从将要执行命令的那个
`zsh -lc` 里读取实际生效值——登录 profile 可能导出了本进程根本看不到的变量——并且把命令里的赋值
视为对它的覆盖，包括 `MAKEFLAGS=` 这种刻意清空的写法。当这个环境完全读不到时，受影响的维度保持
未覆盖并记下原因：未经验证的假设不是证据，而"那个变量大概没设"正是一个假设。

一旦有多个层次都能设置同一个变量，**它们之间的优先级本身就成了检查的一部分**，而且必须实测、
不能想当然。对 GNU make 实测的结果是：

- 重复赋值在两个层次里都是**后者生效**（`MAKEFLAGS=s MAKEFLAGS=n make …` 什么都不跑，反过来
  全跑）——保留*第一个*重复值的实现会读到那个无害的值，从而把一次 dry run 判成通过；
- shell 前缀**替换**继承到的变量（即使导出了 `MAKEFLAGS=n`，`MAKEFLAGS=s make …` 仍会执行）；
- 命令行变量赋值与环境是**累加**关系，而不是覆盖：`MAKEFLAGS=n make MAKEFLAGS=s test` 与
  `MAKEFLAGS=s make MAKEFLAGS=n test` 都什么也不跑，因为哪一层带了 `n`，那个开关就已经生效了。

第三条恰恰是靠直觉会想错的一条——"命令行压过环境变量"是常见约定，而对开关类变量并不成立。
因此这项检查会遍历所有层次、任一层拦住即拒绝；测试矩阵也按两个方向（`s → n` 与 `n → s`）
**先断言 marker 数、再断言判定**，让"听起来对"的优先级模型无法通过。

这里的边界值得说清楚，因为这道门禁的能力到此为止：覆盖表声明的是*一条以该维度为既定用途的
命令确实运行过且退出码为 0*，而不是这条命令有意义。一个 recipe 写成 `@echo skipping` 的
`test:` 目标，不运行语义分析是无法与真目标区分的。可检测的不诚实（有文档定义的"不执行"模式）
会被拒绝；不可检测的不诚实（目标本身在说谎）不是分类器能裁决的问题，而是仓库自身的诚实度问题——
正文里按维度列出的具体命令，就是让评审者能发现它的地方。
...
扫描范围由 `git push` 的实际行为决定：推送分支传输的是范围内的每一个提交，而不是这些提交叠加后的结果——所以"前一个提交加入、后一个提交删除"的凭据仍然会落进远端仓库的历史，再删一次也删不掉。如果只扫 `origin/main...HEAD` 的净差异，这种情况会报告为干净。因此 E 号门禁扫描净差异与 `git rev-list origin/main..HEAD` 中每个提交的并集，把这类命中标注到具体提交，并在范围读不全时 fail closed。白名单同理：豁免只匹配命中的凭据值本身，因为拿整行去匹配，会让行尾注释里一个无关的词（"example"、"sample"）豁免紧挨着它的真实令牌。作用范围大于判断对象，就会 fail open。

"以操作真正传输的内容为判断对象"还推出另外两条：

- **文件路径由 git 给出，不从补丁文本里解析。** 二进制补丁只有一行 `Binary files ... differ`；新增的空文件和纯重命名连 hunk 都没有。这三种情况都不带文本解析所依赖的 `+++` 头，于是历史里的 `.p12` 密钥库或 `id_rsa` 对补丁解析完全不可见。门禁改用 `--name-only -z --diff-filter=ACMR` 枚举，并给所有输出路径的命令加 `-c core.quotePath=false`——因为 git 默认会把非 ASCII 文件名渲染成转义的显示形式，那个形式匹配不到任何东西。至于 git 无论如何都会加引号的名字（含 `"`、`\` 或换行），在解析补丁头时解码。路径匹配失败不会报错，文件只是静悄悄地退出扫描。
- **每一个 push URL 都是一个身份。** 一个 remote 可以配置多个 push URL，一次 `git push` 会写入全部；而 `git remote get-url --push origin` 只返回第一个。校验了第一个 URL 和 `gh` 元数据，并不能说明第二个是什么。`--push --all` 才是完整的写入目标集合，且要放在同一次比较里一起判定。
...
这样设计有一个非常现实的原因，也正是"两者都要查"而不是只查其一的原因。按 GitHub 默认的 squash 提交信息规则：PR 含两个及以上提交时，落到 `main` 的是 PR title 加提交列表；而**只含一个提交时，落下的是该提交自己的标题与正文**，PR title 完全不参与。仓库还可以把格式改为只用 PR title、PR title 加提交明细、或 PR title 加完整描述。所以只查其中一个，无论查哪个都留下了让主干历史变形的真实路径。完整对照见 `references/merge-strategy-guide.md`。
