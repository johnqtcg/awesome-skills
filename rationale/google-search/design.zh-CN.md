...
### 4.14 搜索语法会过期，所以要分级而不是罗列

一个搜索 skill 里最容易过期的内容就是它教的运算符，而这里同时出现了两种过期。

GitHub 有两套搜索引擎，各自的限定词不同。把两套合成一张表，就写出了 `filename:go.mod` 和 `stars:>100` 这样的“代码搜索示例”——这两个限定词在代码搜索里都不存在，而且都不会报错：GitHub 会把无法识别的限定词当成普通检索词，于是查询不是失败，而是安静地返回一堆无关结果。现在两套引擎分开写，并保留一张“不要用”的对照表，列出无效限定词和各自的替代写法。这张表本身是有承重作用的：一旦它被删掉，那条“示例里不许出现无效限定词”的检查就会变成空转。

Google 官方帮助页只列出六个运算符。把 `allintitle:`、`intext:`、`imagesize:`、`related:`、`define:` 和 `site:` 并排展示，等于暗示它们同样可靠，而事实并非如此——`+` 和 `link:` 已经被彻底移除。现在运算符按支持程度分三级：A 级（官方文档列出）、B 级（无文档但通常有效）、C 级（不可靠）。这样证据链就不会建立在一个可能被静默忽略的过滤条件上。

同一节还写清了 `before:` 和 `after:` 到底在过滤什么：文档的最后更新时间，而不是事件发生的时间。一篇 2019 年的教程今天重新发布，就能通过 `after:2026-01-01`；一份 2024 年正确但没人改动过的规范页面反而会被筛掉。把这两个运算符当成“内容是新的”的证据（而不是当成降噪手段），正是一个过时答案获得确定日期的方式。

### 4.15 准备的查询和执行的查询是两笔预算

Quick 模式只允许 2 条查询，而 Workflow 又要求“至少准备三个变体”。这两条规则各自都没错，只是用同一个单位表述，于是变成了矛盾。现在预算只约束**已执行**的查询；变体准备是另一件事，准备了但没跑的变体作为可复用查询交付，并标注 `not run`。这样既保留了第三个变体的价值，又不让一条没执行的查询充当证据——后者正是执行真实性门禁要防的事。

### 4.16 标签集合必须封闭，并且只在一处定义

4.10 的双标签规则要成立，前提是标签取值固定，而它已经漂成了三个版本：Output Contract 列了一套来源层级，`source-evaluation.md` 列了另一套并以“或其他明确命名的层级”结尾，worked example 里又用了 `Medium-High` 和 `Mixed official + practitioner`——这两个值哪张表上都没有。一个开放的枚举不是标签体系，只是命名习惯。

现在两个集合都是封闭的——`High` / `Medium` / `Low`，以及包含 `Practitioner report` 在内的六个来源层级——只在 `source-evaluation.md` 定义，其他文件引用而不复述。当一个数字同时依赖两个层级时，规则是按更弱的那个标注。正是这条规则迫使第二个 worked example 被重写：把一份讲连接池参数语义的官方文档，和一篇博客给出的容量公式凑在一起，并不能让这个公式变成官方结论。诚实的判定是 `Partial`，说明缺的是哪一环，并把公式标为 `Practitioner report` / `Low`。

### 4.17 测试必须能区分“推荐”和“警告”

原来的回归测试是把 `SKILL.md` 和全部 reference 拼成一个大字符串做关键词匹配。这种形状看不见对错：`assert "filename:" in ALL_CONTENT` 在文档推荐这个限定词时通过，在它被移进“不要用”对照表之后照样通过。更糟的是有两条这样的断言让错误变成了承重结构——把语法改对，测试反而会红。

现在检查绑定到包含该结论的最小范围（先文件、再小节），既能要求出现、也能禁止出现，背后还有一个语义 linter，其每一条规则都用一个“把对应缺陷重新植回去”的变异证明过确实会触发。有几条规则的存在只是为了让别的规则保持诚实：一条在“不要用”对照表消失时报错，因为删掉那张表会让它上面那条规则变成空转；另外几条在输入不足以判断时返回 `unknown` 而不是放行——比如报告里没有已执行查询清单可比对，或者一条 GitHub 查询没有声明引擎。看不见检查对象的检查，必须说自己看不见。

### 4.18 意图无法判定时，要求声明，而不是猜

第二轮复审发现 linter 自己也在以它本来要抓的那种形状 fail open。最清楚的一例：`language:go errgroup stars:>100` 检查结果为空。引擎是从限定词**猜**出来的，而 `stars:` 让"repository search"成为显而易见的猜测——在这个读法下这条查询完全合法。但同样这个字符串，也正是一个人想在 star 多的项目里找代码、又不知道代码搜索没有 `stars:` 时会写出来的东西。两种读法与输入的每一个字符都相容。

把探测规则放宽或收紧都解决不了，因为缺失的信息不在查询里，而在作者的意图里。所以现在要求 GitHub 查询带上 `[github-code]` 或 `[github-repo]`，没声明的报 `unknown` 并说明猜了哪个。声明为代码搜索时，同一条查询会正确地在 `stars:` 上报错；声明为仓库搜索时，会正确地通过。

一般化的规则是：当一个输入存在两种同样合法、但导向不同判定的读法时，检查器不能替作者选一个。要么让格式携带这个区分信息，要么给出 `unknown`。默默选一个，正是检查器在它本该发挥作用的那个 case 上报绿的方式。

### 4.19 一个同时变了两个量的 A/B，两个量都没测到

为 4.17 那条行为验证缺口写的前向 eval harness，自己带着它本要暴露的那类缺陷上线了：对照组用的是 `--tools ""`。

- with-skill：加载了 skill，也有搜索工具
- without-skill：没有 skill，**同时连搜索都做不了**

于是差值测的是"skill 加一个搜索工具"对比"模型先验"——对一个**搜索** skill 来说几乎没有意义。对照组无论指令写得多好都不可能给出 URL、也不可能拿真实来源去核对某个限定词，这就保证了那些最像证据的判据上必然出现差值。它看起来严谨，是因为两组被标成了 `with_skill` 和 `without_skill`；这两个标签描述的是意图，不是真正被操纵的变量。

修法是把工具授权抽成一个常量同时传给两组，唯一的差别只剩提示词是否指向 skill，并用一条测试断言 `run_arm` 里没有任何分支按组别改动命令。同时还改了三处小的：零胜场的运行现在退出 INCONCLUSIVE 而不是成功（"没输"不等于"有收益"）；重复采样按多数聚合，而不是相信随机模型的单次输出；调用顺序打乱，让运行期间的抖动无法系统性地落在某一组上。

修正后的第一次运行结果值得记下来，因为它并不好看。在一个纯事实查询、以及 `before:`/`after:` 语义那道题上，对照组——同模型、同工具、没有 skill——给出了正确答案并附了来源，其中一处的措辞几乎和 skill 教的一样。这类判据的结论是 **uninformative**，而 uninformative 就是诚实的答案：在搜索能力持平的前提下，对一个有能力的模型本来就能完成的事实查询，这个 skill 没有可测量的增量。它的价值只能到纪律真正起作用的地方去找——没有来源时拒绝作答、没打开过的页面拒绝转述、以及把一个数字依赖什么讲清楚。反过来去凑一组能让 skill 赢的场景，很容易做到，但毫无价值。
