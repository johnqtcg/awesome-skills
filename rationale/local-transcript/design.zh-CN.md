...
### 4.9 LLM 校对改为按需开启（2026-08-02 修订）

skill 最初对中文默认开启 LLM 校对、对英文默认关闭，理由是中文 ASR 的同音字、近音词错误更集中，收益更大。关于**错误集中在哪里**的判断是对的；但"本地模型能修好这些错误"这个前提从未被测量过，等到真去测，它不成立。

用 `local` 后端（`Qwen2.5-7B-Instruct-4bit`）在三段 30 分钟中文播客转写上的实测：

- 人工找出的约 130 处同音错，它一处也没改对（需求策/需求侧、地域难度/地狱难度、天王/天网、万物被裹/万物百吉饼……）
- 两次运行中共有三块未通过输出校验，重试耗尽后退回未校对原文
- 有一块把提示词里的 `待校对文本:` 标签抄进了正文（现已被校验拦下，但它确实发布出去过）
- 510 秒的一次运行里它占了 350 秒，仍是主要时间成本，而与之相抵的收益没有测到

因此 `--llm-backend` 默认值改为 `none`。两个后端仍可按次开启，`--llm-proofread-en` 依旧是英文在后端选择之上的第二道开关。

这个结论的适用范围：三段转写、一位说话人、一个后端。它不是对 LLM 校对的整体判决，`claude` 后端也从未测过。但足以让我们停止默认支付这份成本——举证责任在功能一方，不在事后收拾残局的使用者。

这件事底下还压着另一个教训。同一批运行里每份转写出现 128～185 处 `,，`，当时看起来也是 LLM 造成的破坏，并被这样引述过。实际不是：`join_lines()` 把半角逗号当成"没有标点"，于是又补了一个全角逗号。把流水线自己的 bug 记在模型头上，差一点让一个真实缺陷被掩埋——这也是本节修订时要把**实测到的模型行为**和**我们自己制造的产物**分开写的原因。
