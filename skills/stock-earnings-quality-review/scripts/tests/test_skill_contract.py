"""Zero-LLM contract tests for stock-earnings-quality-review worker skill."""
...
MAX_SKILL_LINES = 500
SKILL_NAME = "stock-earnings-quality-review"
REQUIRED_REFERENCES = ["earnings-quality-patterns.md"]
REQUIRED_SECTIONS = [
    "Purpose",
    "When To Use",
    "When NOT To Use",
...
def test_quantification_discipline(skill_text: str) -> None:
    """Earnings-quality worker must mandate numerical evidence."""
    assert (
        "numerical" in skill_text.lower()
        or "quantif" in skill_text.lower()
        or "compute" in skill_text.lower()
    ), "earnings-quality worker must require quantified findings"