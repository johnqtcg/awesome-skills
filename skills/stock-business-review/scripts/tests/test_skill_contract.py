"""Zero-LLM contract tests for stock-business-review worker skill."""
...
MAX_SKILL_LINES = 500
SKILL_NAME = "stock-business-review"
REQUIRED_REFERENCES = ["business-model-patterns.md"]
REQUIRED_SECTIONS = [
    "Purpose",
    "When To Use",
    "When NOT To Use",
...
def test_four_mandatory_gates(skill_text: str) -> None:
    """Each worker must have 4 mandatory gates."""
    assert "Execution Integrity Gate" in skill_text
    assert "Non-US Refusal Gate" in skill_text or "Citation Gate" in skill_text
...
def test_references_exist() -> None:
    for ref in REQUIRED_REFERENCES:
        ...
        assert path.stat().st_size > 500, f"reference {ref} too small"


def test_references_loaded(skill_text: str) -> None:
    for ref in REQUIRED_REFERENCES:
        assert ref in skill_text, f"reference {ref} not loaded in SKILL.md"


def test_finding_prefix_present(skill_text: str) -> None:
    assert FINDING_ID_PREFIX in skill_text, (
        f"worker must use Finding ID prefix {FINDING_ID_PREFIX}"
    )


def test_dispatched_by_orchestrator(skill_text: str) -> None:
    assert "stock-analysis-lead" in skill_text, (
        "worker should reference the orchestrator that dispatches it"
    )


def test_does_not_recommend_verdict(skill_text: str) -> None:
    """Workers must not give buy/sell/hold verdicts; the orchestrator does that."""
    # Negative routing must be present
