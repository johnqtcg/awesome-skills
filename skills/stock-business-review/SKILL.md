...
```findings-json
{
  ...
  "positives": [],
  "data_gaps": [],
  "checklist_coverage": {"items_total": 11, "items_checked": 0, "items_not_found": 0, "ids_not_checked": []},
...
- Every finding ID must start with `BUS` — the prefix is the whole segment
...
- `checklist_coverage`: `items_checked + items_not_found` must reach
  `items_total` (11), so "the checklist ran to completion" is verifiable.
...
python3 <path-to>/stock-analysis-lead/scripts/finlib/worker_contract.py \
  validate --reply <this-reply>.md --expect-worker stock-business-reviewer
```
...
You are one of the orchestrator's parallel workers (the fan-out size is depth- and trigger-dependent — see `stock-analysis-lead/references/dispatch-protocol.md`). The orchestrator depends on you reporting only what you actually found in filings, with full citations. Over-reporting wastes the orchestrator's deduplication budget; fabrication breaks the entire analysis. When in doubt, suppress.