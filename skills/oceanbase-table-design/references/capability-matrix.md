...
2. **~~The exact interaction between `DROP` / `TRUNCATE PARTITION` and global indexes.~~ RESOLVED against the official Oracle-mode documentation — see `partitioning.md` §4.1 for the full rule.** Summary: the keyword `UPDATE GLOBAL INDEXES` is mandatory, and *even with it* the lazy-maintenance optimisation is skipped in three named cases, the first of which — **a HASH first-level partition** — is decision-determining for composite designs. **Still open within this item**: the tenant hidden config `_ob_enable_truncate_partition_preserve_global_index` is documented only on the MySQL-mode page, and how it composes with the Oracle-mode exclusion list is not stated anywhere; and the target here is V4.5.0 while the sourced pages are V4.4.2 / V4.3.5.
