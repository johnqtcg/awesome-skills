#!/usr/bin/env python3
"""Skill-output eval for thirdparty-api-integration-test: a GRADER for what a model produces
when driven by this skill, plus fixtures and a self-test proving the grader discriminates.

Closes COVERAGE.md Known Gap 1. The existing layers are:

  * contract      — the rule TEXT exists, and the doc/fixture helper bodies are identical;
  * behavioral    — a fixture THIS REPO wrote behaves correctly, and the runner script does;

Neither looks at a response. A model could name every gate, copy the scorecard, claim 4/4
Critical, and emit a test that `t.Skip`s on missing config and trusts `ENV` alone — the exact
false-green this skill exists to prevent against a **billable** vendor — and score perfectly.

So the grader RUNS the emitted test. Each fixture declares an `env_matrix`: environments plus
the outcome the skill's §Skip vs Fail rules require of each.

    gate unset ................................. must SKIP
    gate on, a required var missing ............ must REFUSE
    gate on, ENV=production .................... must REFUSE
    gate on, bare host with no scheme .......... must REFUSE
    gate on, no VENDOR_SANDBOX_HOSTS ........... must REFUSE
    gate on, host off VENDOR_SANDBOX_HOSTS ..... must REFUSE
    gate on, no VENDOR_TEST_ACCOUNTS ........... must REFUSE
    gate on, account off the allowlist ......... must REFUSE
    gate on, valid sandbox ..................... must REACH THE CALL

"REFUSE" is stronger than "fail": the run must fail *without attempting the call*. A plain
`fail` is satisfied by a response that ignores the sandbox allowlist, dials the vendor and
fails on connect — scoring a correct refusal for contacting a live paid API. And the last row
is the clean arm: without it a test that `t.Fatalf`s unconditionally satisfies all eight
refusals and scores perfectly. Every host is loopback or an undiallable bare name, so the
matrix cannot reach anything real even when the response is wrong.

The second fixture grades the **refusal** path — the Scope Validation Gate's hard stop on a
first-party handler — because a skill whose first gate can say "STOP, use $api-integration-test"
must be graded on stopping.

NOTE ON PROVENANCE: this is a **parallel** implementation of the sibling
`$api-integration-test`'s grader, not a shared file. Same structure, different fixtures and
different gate vocabulary; edits here do not propagate there. (The same rule this skill states
for its `common-*` references.)

Honesty: this proves the GRADER separates the hand-authored good and bad exemplars. It does
not prove a live model passes — that is the opt-in `LiveSkillEval`, skipped unless
`THIRDPARTY_SKILL_EVAL_CMD` is set.
"""
...
LIVE_CMD = os.environ.get("THIRDPARTY_SKILL_EVAL_CMD")
...
def _go_env(root: str) -> dict:
    ...
    for k in list(env):
        if k.startswith(("THIRDPARTY_", "INTEGRATION_", "VENDOR_")) or k == "ENV":
            env.pop(k)
    return env
...
class _GoRunner:
    ...
    def _module(self, source: str, go_test: str) -> str:
        try:
            root = tempfile.mkdtemp(prefix="tp-skilleval-")
...
class SkillOutputGraderTests(unittest.TestCase):
    ...
    def test_at_least_two_fixtures_and_both_paths_are_covered(self):
        ...
        self.assertEqual({"in_scope", "out_of_scope"}, scopes,
                         "the corpus must grade both the authoring path and the scope hard stop")
    ...
    def test_the_plausible_bad_response_fails_only_on_behaviour(self):
        """`vendor_charge_http/bad.md` runs every gate, declares the right scope, mode and
        ...
        d = os.path.join(EVAL_ROOT, "vendor_charge_http")
        ...
        joined = " | ".join(reasons)
        for name in ("required var missing", "not on VENDOR_SANDBOX_HOSTS",
                     "no VENDOR_TEST_ACCOUNTS", "account not on the allowlist"):
    ...
    def test_the_good_exemplar_is_not_passing_by_refusing_everything(self):
        ...
        always_fatal = (
            ...
            '\tt.Fatalf("refuse production/live vendor target")\n}\n')
...
class GraderUnitTests(unittest.TestCase):
    ...
    def test_the_in_scope_matrix_covers_every_skip_vs_fail_row(self):
        ...
        fx = load_fixture(os.path.join(EVAL_ROOT, "vendor_charge_http"))
    ...
    def test_matrix_environments_never_leak_a_real_gate_variable(self):
        ...
        env = _go_env("/tmp/x")
        for leaked in ("THIRDPARTY_INTEGRATION", "VENDOR_SANDBOX_HOSTS",
                       "VENDOR_TEST_ACCOUNTS", "ENV", "INTEGRATION_ALLOW_PROD"):
            self.assertNotIn(leaked, env, f"{leaked} leaks from the parent environment")


@unittest.skipUnless(
    LIVE_CMD and GO,
    "set THIRDPARTY_SKILL_EVAL_CMD to a shell command that reads a prompt on stdin "
    ...
    def test_live_model_output_passes_grader(self):
        ...
        for d in fixture_dirs():
            ...
            with self.subTest(fixture=fx["id"]):
                prompt = (
                    "Follow this thirdparty-thirdparty-api-integration-test skill exactly and produce its full "
