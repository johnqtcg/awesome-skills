"""Zero-LLM contract tests for stock-industry-review worker skill."""
...
MAX_SKILL_LINES = 500
SKILL_NAME = "stock-industry-review"
REQUIRED_REFERENCES = ["moat-typology.md"]
REQUIRED_SECTIONS = [
    "Purpose",
    "When To Use",
    "When NOT To Use",
...
def test_seven_moat_types_referenced(skill_text: str) -> None:
    """Industry worker must reference the 7-type moat taxonomy."""
    moat_types = ("network", "brand", "scale", "switching", "patent", "regulatory", "data")
    found = sum(1 for t in moat_types if t in skill_text.lower())
    assert found >= 5, (
        f"industry worker should reference most moat types; only found {found}/7"
    )